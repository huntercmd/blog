...
def Add_session(mdfile):#输入md文件，提取二级session标题，放到源文档中
	...
	for line in f.readlines():
		if re.search("^#{2}[0-9a-zA-Z\s]+",line):#发现二级标题
			toctext.append(line)
			toclist.append("- [%s](#%s)" %(line[2:].strip(),line[2:].strip()))  #去掉##符号
			n += 1 #统计几个session, 默认有一个
	f.close( )

	oldmd = open( mdfile, 'r').read()
	if n > 1:#如果存在session
		newlist = sorted(toclist[1:])
		#oldmd = oldmd.replace(toctext[0], toctext[0]+"\n".join(toclist[1:])+"\n\n")
		oldmd = oldmd.replace(toctext[0], toctext[0]+"\n".join(newlist)+"\n\n")
...
if __name__=='__main__': 
	...
	outdir = os.getcwd()[:-3]+"posts_server" #生成文件的路径
...
#生成各html文件，统计目录
	for item in mdlist:
		...
		#context_md = f.read()
		context_md = Add_session(mddir+"\\"+item)#添加二级标题索引
